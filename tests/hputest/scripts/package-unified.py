...
def workload_rows(release, mode):
    ...
    if (published, blocked) != (40, 20):
        raise ValueError(f"{mode} workloads require 40 published and 20 blocked cases")
...
def package(workloads, silent_workloads, subtests, silent_subtests, output_root):
    ...
    try:
        ...
        if (published, blocked, len(indexes)) != (136, 20, 156):
        ...
        (package_root / "MANIFEST.txt").write_text(
            ...
            "published_test_identities=68\npublished_variant_sets=136\n"
            "blocked_index_only=20\nqualification=BUILD_READY_NOT_IT_PASS\n",
            encoding="utf-8")
    ...
    print(f"Unified HPU package: {output / 'release' / 'hputest'}; "
          "68 tests x 2 variants, 20 blocked index-only")
