...
def workload_rows(release, mode):
    ...
    if (published, blocked) != (44, 16):
        raise ValueError(f"{mode} workloads require 44 published and 16 blocked cases")
...
def package(workloads, silent_workloads, subtests, silent_subtests, output_root):
    ...
    try:
        ...
        if (published, blocked, len(indexes)) != (144, 16, 160):
        ...
        (package_root / "MANIFEST.txt").write_text(
            ...
            "published_test_identities=72\npublished_variant_sets=144\n"
            "blocked_index_only=16\nqualification=BUILD_READY_NOT_IT_PASS\n",
            encoding="utf-8")
    ...
    print(f"Unified HPU package: {output / 'release' / 'hputest'}; "
          "72 tests x 2 variants, 16 blocked index-only")
