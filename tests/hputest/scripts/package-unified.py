...
def workload_rows(release, mode):
    ...
    if (published, blocked) != (48, 14):
        raise ValueError(f"{mode} workloads require 48 published and 14 blocked cases")
...
def package(workloads, silent_workloads, subtests, silent_subtests, output_root):
    ...
    try:
        ...
        if (published, blocked, len(indexes)) != (152, 14, 166):
        ...
        (package_root / "MANIFEST.txt").write_text(
            ...
            "published_test_identities=76\npublished_variant_sets=152\n"
            "blocked_index_only=14\nqualification=BUILD_READY_NOT_IT_PASS\n",
            encoding="utf-8")
    ...
    print(f"Unified HPU package: {output / 'release' / 'hputest'}; "
          "76 tests x 2 variants, 14 blocked index-only")
