...
def workload_rows(release, mode):
    ...
    if (published, blocked) != (47, 15):
        raise ValueError(f"{mode} workloads require 47 published and 15 blocked cases")
...
def package(workloads, silent_workloads, subtests, silent_subtests, output_root):
    ...
    try:
        ...
        if (published, blocked, len(indexes)) != (150, 15, 165):
        ...
        (package_root / "MANIFEST.txt").write_text(
            ...
            "published_test_identities=75\npublished_variant_sets=150\n"
            "blocked_index_only=15\nqualification=BUILD_READY_NOT_IT_PASS\n",
            encoding="utf-8")
    ...
    print(f"Unified HPU package: {output / 'release' / 'hputest'}; "
          "75 tests x 2 variants, 15 blocked index-only")
