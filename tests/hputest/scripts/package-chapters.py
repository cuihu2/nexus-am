...
DIAGNOSTIC_IDS = (
    ...
    {"HPU_IT_DIR_CMB_009", "HPU_IT_DIR_CMB_010",
     "HPU_IT_DIR_CMB_012", "HPU_IT_DIR_CMB_013", "HPU_IT_DIR_CMB_014"}
)
...
def package(artifact, require_all=False, require_diagnostic=False, require_silent=False):
    ...
    if require_diagnostic:
        if seen_ids != DIAGNOSTIC_IDS or any(
                ...
                for row in indexes):
            raise ValueError("diagnostic package must contain exactly the nineteen ready 03/04 cases")
    ...
    try:
        ...
        uart_note = (
            ...
            "本包只包含03的九个用例以及04的BConv、整体NTT、整体INTT、KeySwitch、Auto，共14个ELF。"
...
def main():
    ...
    mode.add_argument("--diagnostic", action="store_true", help="要求03/04十八项全量UART诊断构建")
