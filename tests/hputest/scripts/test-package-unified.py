...
class UnifiedPackageTests(unittest.TestCase):
    ...
    def test_one_tree_contains_normal_and_silent_and_replaces_03_parents(self):
        ...
        self.assertEqual(len(list(package.rglob("*.elf"))), 136)
        ...
        self.assertEqual(sum(bool(row["elf"]) for row in rows), 136)
        self.assertEqual(sum(row["publish_status"] == "BLOCKED_NOT_PUBLISHED" for row in rows), 20)
