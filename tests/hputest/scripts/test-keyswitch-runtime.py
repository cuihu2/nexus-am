#!/usr/bin/env python3
"""Host checks for the KeySwitch fixture/runtime wiring; no HPU command is issued."""
...
const uint32_t keyswitch_window[KEYSWITCH_TOTAL_WORDS] = {0};
const uint32_t keyswitch_golden[KEYSWITCH_COMPONENTS * KEYSWITCH_Q_COUNT * KEYSWITCH_N] = {0};
static uint32_t memory[KEYSWITCH_TOTAL_WORDS];

volatile uint32_t *ddr_line(unsigned line) {
    return memory + line * WORDS_PER_LINE;
}
...
int main(void) {
    if (keyswitch_prepare() != 0 || keyswitch_check_results() != 0 ||
        keyswitch_check_memory() != 0) return 1;

    memory[KEYSWITCH_OUTPUT_OFFSET * WORDS_PER_LINE + 7] = 1;
    if (keyswitch_check_results() == 0 || keyswitch_check_memory() != 0) return 2;
    memory[KEYSWITCH_OUTPUT_OFFSET * WORDS_PER_LINE + 7] = 0;

    memory[19] = 1;
    if (keyswitch_check_memory() == 0) return 3;
    memory[19] = 0;

    memory[KEYSWITCH_GUARD_OFFSET * WORDS_PER_LINE + 23] = 1;
    if (keyswitch_check_memory() == 0) return 4;
    memory[KEYSWITCH_GUARD_OFFSET * WORDS_PER_LINE + 23] = 0;

    memory[KEYSWITCH_SCRATCH_OFFSET * WORDS_PER_LINE + 11] = 1;
    if (keyswitch_check_memory() != 0) return 5;
    return 0;
}
"""


class KeySwitchRuntimeTests(unittest.TestCase):
    delivery = None
    ...
    def test_output_and_scratch_are_writable_but_input_and_guard_are_not(self):
        with tempfile.TemporaryDirectory(prefix="keyswitch-runtime-") as directory:
            root = Path(directory)
            ...
            subprocess.run([
                ...
                str(ROOT / "src/common/it_keyswitch.c"), str(harness),
                "-o", str(executable),
            ], check=True)
    ...
    def test_case_and_generated_program_are_warning_clean_c(self):
        if self.delivery is None or not (self.delivery / "keyswitch_delivery.h").is_file():
            self.fail("pass --delivery pointing at generated keyswitch-data")
        with tempfile.TemporaryDirectory(prefix="keyswitch-syntax-") as directory:
...
if __name__ == "__main__":
    ...
    KeySwitchRuntimeTests.delivery = args.delivery.resolve(strict=True)
