...
int main(void) {
    ...
    for (unsigned variant = 0U; variant < 4U; ++variant) {
        ...
        LOG_DEBUG("[HPU][PADD][ROUND] variant=%u profile=%u dst=p%u "
        ...
        LOG_DEBUG("[HPU][PADD][ISSUE] mod -> A/B -> PADD%s -> "
        ...
        const int data_rc = v2_check_words("PADD", LINE_OUT, golden, POLY_WORDS, MOD_Q0);
        phase_mark("check-guard");
        const int guard_rc = v2_check_memory("readonly-and-guard");
        if (data_rc != 0 || guard_rc != 0)
            return case_fail(__FILE__, __LINE__);
        phase_mark("round-done");
        LOG_DEBUG("[HPU][PADD][ROUND-PASS] variant=%u compared=%u "
