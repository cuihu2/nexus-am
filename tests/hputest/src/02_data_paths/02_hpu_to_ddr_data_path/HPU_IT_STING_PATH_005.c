...
/*
 * 测试点：IT-PATH-005
 * 目的：两个对象、多个 DDR 区域的隔离；完整容量上界和随机时序仍需外部验证。
 * 模式：STING约束随机（P1）。
 * 外部条件：
 *   - HPU_REQ_IT_MONITOR
...
int main(void) {
    ...
    if (v2_allow_output(LINE_OUT, POLY_LINES) != 0 ||
        ...
        return failure(__LINE__, phase);
    phase = "object-region-isolation";
    LOG_DEBUG("[HPU][ISSUE] seed_tag=0x%x DLOAD p0 A=%u, p1 B=%u -> "
           "DSTORE p1 OUT_B=%u, p0 OUT=%u count=%u -> PSYNC; "
           "resident_objects=2,not-capacity-limit\n",
           seed, LINE_A, LINE_B, LINE_OUT_B, LINE_OUT, POLY_LINES);
