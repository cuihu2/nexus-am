...
int main(void) {
    ...
    if (v2_allow_output(LINE_OUT, POLY_LINES) != 0)
        return failure(__LINE__, phase);
    phase = "busy-state-transition";
    LOG_DEBUG("[HPU][ISSUE] DLOAD p0 A=%u -> DSTORE p0 OUT=%u count=%u -> PSYNC; "
           "expected=busy-observed,then-idle-and-IRQ\n", LINE_A, LINE_OUT, POLY_LINES);
    ...
    for (timeout = 0U; timeout < TIMEOUT; ++timeout) {
        ...
        if ((status & STATUS_FAULT) != 0U) return failure(__LINE__, phase);
        if ((status & STATUS_BUSY) != 0U) saw_busy = 1;
        if (saw_busy && (status & STATUS_BUSY) == 0U &&
            (csr_read(CSR_IRQ) & IRQ_LEVEL) != 0U)
            break;
    }
    LOG_DEBUG("[HPU][STATE] sampled_status=0x%x saw_busy=%d poll_count=%u limit=%u\n",
           status, saw_busy, timeout, TIMEOUT);
    if (timeout == TIMEOUT || !saw_busy) return failure(__LINE__, phase);
    if (check_status() != 0) return failure(__LINE__, phase);
