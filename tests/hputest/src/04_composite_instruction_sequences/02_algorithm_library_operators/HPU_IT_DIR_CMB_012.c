...
#include <ckks_reline.h>

/*
 * 测试点：IT-CMB-012
 * 目的：通过固定 HPU_SEAL CKKS 算法库接口验证独立 Reline。
 * 输入为 N4096/Q4|P1、canonical-NTT 的三分量乘积密文；程序只包含
 * CkksOperationPlan::append_relinearize 生成的三分量到二分量重线性化，
 * 密钥、KeySwitch 常量、scratch、DMA 重定位和精确 SEAL golden 均来自
 * 同一次 producer。输入转换区及输出是唯一可写区域，其余常量和尾部 guard
 * 必须保持不变。
 */
int main(void) {
    int rc;
    ...
    LOG_DEBUG("[HPU][CKKS][RELINE][ISSUE] "
              "api=hpu::seal_adapter::CkksOperationPlan::append_relinearize "
              "N=%u Q=%u inputs=3 outputs=2\n", CKKS_N, CKKS_Q);
    phase_mark("issue");
    rc = hpu_run_ckks_reline();
    if (rc != 0) {
        LOG_ERROR("[HPU][CKKS][RELINE][FAIL] phase=producer-program rc=%d\n", rc);
        return case_fail(__FILE__, __LINE__);
    }
