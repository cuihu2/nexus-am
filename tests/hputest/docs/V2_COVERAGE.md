...
**本次不是“全部测试点已完成”或“全部 IT 已通过”。** 51 个后续用例中，35 个具备真实软件自检；
...
inline-asm legacy-main `6903096`，CMB_009/010及CKKS应用另用固定HPU_SEAL main
`b5398a3`，两套编码不混用。
...
| CMB_011..015 | 未接入真实算法库API | 需固定接口、参数、密钥、数据、精度和golden |
| STING_CMB_007 | 未接入随机长链 | STING入口、seed重放、对象分配与逐阶段golden |

整体NTT/INTT独立用640line窗口；BConv用2048line窗口；KeySwitch用14721line窗口；Auto保留producer完整窗口并添加64-line尾部guard；HADD/HMUL分别用1601/32899line窗口。
...
CMB_010同样不以单条PMUL冒充算法库密文乘法：固定planner同时准备重线性化密钥、KeySwitch/
BEHZ常量和canonical twiddle，producer先用modified-SEAL与HPU software executor逐系数对拍，
AM再执行同一完整程序并比较两分量Q4 golden。PMUL基础功能仍由03-003覆盖。
...
03用37个独立subtest替换九个串行整例，其余章节保留35组非占位workload；每项同时提供普通和
`_silent`版本，共144组ELF/BIN/反汇编。16项未就绪只列原因，不夹在可运行列表中。
