...
[V2_COVERAGE.md](docs/V2_COVERAGE.md)。后续49项中33项有软件自检，16项尚未接入；
...
└── generated/                  # selected semantic imports and source staging
...
The `riscv64-xs` platform uses the PLIC
window at `0x3c000000`, shared through the platform `PLIC_BASE_ADDR` in
`am/src/xs/include/xs.h`. The old `0x04000000` routes to the timer window in
the current IT address map and must not be used for PLIC accesses.
When the AM runtime itself is compiled with `LINKNAN_HPU_IT`, its
pre-S-mode PLIC initialization establishes an
...
`irq_open()` then directly reads back priority `0x3c000404`, S-context
enable word 8 at `0x3c0020a0`, and threshold `0x3c201000`; a map or setup
mismatch therefore fails before `irq_wait()`.  The handler claims and completes
source 257 at `0x3c201004`. `irq_rearm()` retains the same configured source
and context; it does not relocate or reinitialize PLIC. A priority-readback
failure in `irq_open()` occurs before PSYNC and cannot establish a rearm bug.
...
输入、密钥、常量和SEAL golden，APP_002/003则接收上游CKKS应用交付。
...
整体NTT/INTT/BConv/KeySwitch/Auto/HADD/HMUL改用各自的完整数据、常量和golden交付，不用MM输入冒充算子数据。
...
35个后续源码具有真实软件自检。03的PNTT/PINTT已补齐；04的BConv Q→P、整体NTT/INTT、
KeySwitch、Auto、BFV HADD和融合HMUL/relinearize已接入完整producer序列及golden，但仅覆盖各自固定基础组合。
其它16项仍未接入，原因逐项记录于 [blocked.tsv](blocked.tsv)，其中既有缺外部接口，
也有AM接收工作未实现，不能笼统归因于上游“没有数据”。CMB_009/010验收固定
HPU_SEAL API；基础PADD/PMUL覆盖仍保留在03-001/003。
...
共72个测试身份、144组ELF/BIN/TXT；16项未就绪只保留索引。`INDEX.tsv`列出模式和真实路径。
产物保留7天，不提交二进制到Git。
