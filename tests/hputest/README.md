...
`hpu_csr_write32(HPU_CSR_BASE_LO_ADDR, value)`, instead of being hidden in a
combined initialization function.  Instructions remain equally visible as
...
STATUS is idle.  A `csrr`-based HPU-status case is deliberately absent: the
current hardware documentation defines the HPU register window only through
MMIO and provides no architectural HPU CSR number, privilege level, or idle
encoding.  Standard RISC-V interrupt-control CSRs used by case 05 do not form
a second HPU-status access path.

The old ten-row draft is not another supported numbering scheme.  Its two
“CSR/MMIO CSR” pairs were duplicates and were merged into cases 03 and 06;
the former rows 08/09/10 have no source alias or build artifact.
...
4. Case 03 observes DLOAD `busy` go high and then low.  Cases 04 and 05 both
   ...
   handler and a `volatile` flag.  These prove more than register readback,
   but not payload accuracy.
